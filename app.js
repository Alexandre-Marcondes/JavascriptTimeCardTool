...
function evaluateAllRows() {
  ...
  parsedRows.forEach((row, idx) => {
    ...
    // ---------------------------------------------------------------------
    // 0) Time sequence validation (if all four punches exist)
    // ---------------------------------------------------------------------
    const timeInMinutes = parseTimeToMinutes(row["TIME IN"]);
    const lunchStartMinutes = parseTimeToMinutes(row["LUNCH START"]);
    const lunchEndMinutes = parseTimeToMinutes(row["LUNCH END"]);
    const timeOutMinutes = parseTimeToMinutes(row["TIME OUT"]);
    ...
    if (
      ...
      if (
        !(
          timeInMinutes < lunchStartMinutes &&
          lunchStartMinutes < lunchEndMinutes &&
          lunchEndMinutes < timeOutMinutes
        )
      ) {
        hasError = true;
        errorMessages.push("Invalid time order: punches out of sequence.");
      }
    }
    ...
    // ---------------------------------------------------------------------
    // 2) REGULAR HOURS vs punched time (only if punches exist)
    // ---------------------------------------------------------------------
    if (hasPunches) {
      ...
      // No punches and no sick: then any regular/OT is suspicious
      if (regHours > 0 || otHours > 0) {
        hasError = true;
        errorMessages.push(
          "No punches for this day, but regular and/or overtime hours were entered."
        );
      }
    }

    // ---------------------------------------------------------------------
    // 3) Sick-only day sanity (Rule D2)
    // ---------------------------------------------------------------------
    if (!hasPunches && sickHours > 0) {
      // Pure sick day should not have regular or OT
      if (regHours > 0 || otHours > 0) {
        hasError = true;
        errorMessages.push(
          "Pure sick day should not have regular or overtime hours."
        );
      }
      // Sick day may not exceed 8 hours
      if (sickHours > 8 + epsilon) {
        hasError = true;
        errorMessages.push("Sick hours exceed 8 on a full sick day.");
      }
    }

    // ---------------------------------------------------------------------
    // 4) Mixed sick + work <= 8 (Rule D1)
    // ---------------------------------------------------------------------
    if (hasPunches && sickHours > 0) {
      if (timeHours + sickHours > 8 + epsilon) {
        hasError = true;
        errorMessages.push(
          "Worked hours + sick hours exceed 8 for a single day."
        );
      }
    }

    // ---------------------------------------------------------------------
    // 5) Sick + OT same day not allowed (Rule D3)
    // ---------------------------------------------------------------------
    if (sickHours > 0 && otHours > 0) {
      hasError = true;
      errorMessages.push(
        "Overtime cannot be claimed on a day with sick hours."
      );
    }

    // ---------------------------------------------------------------------
    // 6) Policy-specific rules
    // ---------------------------------------------------------------------
...
// DAILY OT rules for Lu (8-hour daily threshold)
function evaluateDailyOTPolicy({ timeHours, sickHours, regHours, otHours }) {
  ...
  if (!hasPunches) {
    // No punches, overtime shouldn't appear
    if (otHours > 0) {
      errorMessages.push(
        "Daily-OT policy: no punches, overtime should not be entered."
      );
    }
    return { errorMessages };
  }

  // If sick is present, daily sick rules already applied above;
  // we only add daily-OT-specific checks when there is no sick.
  if (sickHours > 0) {
    // All sick interaction rules are handled globally; no extra daily-OT logic here
    return { errorMessages };
  }

  // No sick; now daily-OT logic:
  if (timeHours <= 8 + epsilon) {
    if (otHours > 0) {
      errorMessages.push(
        "Daily-OT policy: overtime present but worked hours are 8 or less."
      );
    }
  } else {
    // Worked more than 8 hours, calculate expected breakdown
    const expectedReg = 8;
    const expectedOT = timeHours - 8;

    if (
      Math.abs(regHours - expectedReg) > epsilon ||
      Math.abs(otHours - expectedOT) > epsilon
    ) {
      errorMessages.push(
        `Daily-OT policy: expected 8.00 regular and ${formatHours(
          expectedOT
        )} overtime based on punches.`
      );
    }
  }
...
function evaluateWeeklyPolicyDaily({ timeHours, sickHours, regHours, otHours }) {
  ...
  if (hasPunches) {
    if (timeHours <= epsilon && otHours > 0) {
      errorMessages.push(
        "Overtime is entered but there are no worked hours for this day."
      );
    }
  } else {
    if (otHours > 0) {
      errorMessages.push(
        "No punches for this day, but overtime hours were entered."
      );
    }
  }
...
// Weekly OT checks (weekly OT policy employees, 40-hour rule)
function applyWeeklyOTChecks(dayRowIndices) {
  ...
  for (const [weekIndex, bucket] of weeks.entries()) {
    ...
    // Weekly OT based only on worked hours (sick does not erase OT)
    const expectedOT = Math.max(workedHours - 40, 0);

    if (Math.abs(otHours - expectedOT) > epsilon) {
      let msg;

      if (expectedOT <= epsilon && otHours > epsilon) {
        // No OT should exist
        msg = `Weekly OT rule: week ${weekIndex} worked = ${formatHours(
          workedHours
        )}h, so overtime should be 0 (found ${formatHours(otHours)}h).`;
      } else if (expectedOT > epsilon && otHours <= epsilon) {
        // OT missing entirely
        msg = `Weekly OT rule: week ${weekIndex} worked = ${formatHours(
          workedHours
        )}h, expected overtime = ${formatHours(
          expectedOT
        )}h but none was entered.`;
      } else {
        ...
        msg = `Weekly OT rule: week ${weekIndex} worked = ${formatHours(
          ...
          expectedOT
        )}h but found ${formatHours(otHours)}h.`;
      }
...
function populatePrintView() {
  ...
  // --- "Original" timecard layout (no computed / no rule error) ------
  if (pOriginalTable) {
  ...
  const summaryHtml = (() => {
    ...
    let wsHtml =
      "<table><thead><tr><th>Week</th><th>Worked</th><th>Sick</th><th>Regular</th><th>OT</th></tr></thead><tbody>";
    weekly.forEach((w) => {
      wsHtml += `
        <tr>
          <td>${w.weekIndex}</td>
